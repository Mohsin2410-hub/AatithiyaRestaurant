...
export class RestaurantDetailsComponent {
  ...
  constructor(
    ...
    this.imgUpload.getAllImgByCategory(3).subscribe(m => {
      ...
      .map(
        d => {
          if (d.isMainDisp)
          {
            ...
            this.tmp.push({
              id: count,
              imgCategory: "Restaurant",
              imgTitle: d.imgTitle,
              imgUrl: d.imgUrl ? d.imgUrl : "Default image called",
              imgLongUrl: d.imgUrl ? `${url}/Restaurant_img/${d.imgUrl}` : this.defaultImg[count-1]
            })
          }
        }
      )
  ...
  uploadImg(fName: string) {
    ...
    this.formData.append("ImgCategory", "3");
    ...
    this.imgUpload.getAllImgByCategory(3).subscribe({
      next: (m) => {
